...
struct LiveView: View {
    ...
    private var tileBackground: Color {
        ...
        if sessionManager.isDemoMode {
            return String(format: "%.0f", max(0, sessionManager.demoSpeedKmh))
        }
        guard case .connected = raceBoxGPS.state else { return "--" }
        ...
        var body: some View {
            NavigationStack {
                ZStack {
                    ...
                    if purchaseManager.hasLiveAccess && sessionManager.isRaceStartArmed {
                        ZStack {
                    ...
                    // MARK: - LiveView Paywall Gate (MVP)
                    if !purchaseManager.hasLiveAccess {
                        Color.black.ignoresSafeArea()

                        VStack(spacing: 16) {
                            Text("LiveView Premium")
                                .font(.system(size: 34, weight: .bold, design: .rounded))
                                .foregroundStyle(.white)

                            Text("Accès requis pour démarrer une course.\nDébloque la LiveView pour utiliser l’écran de course.")
                                .font(.headline)
            ...
            .sheet(isPresented: $isPaywallPresented) {
                LivePaywallView(
                    ...
                    onUnlockNow: {
                        purchaseManager.grantLiveAccessForNow()
                        sessionManager.isRaceStartArmed = false
                        isPaywallPresented = false
                        // optionnel : amène vers le setup
...
private struct LivePaywallView: View {
    let onClose: () -> Void
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 18) {
                ...
                Text("Débloquer la LiveView")
                    .font(.system(size: 28, weight: .bold))

                Text("Accès premium requis pour l’écran de course.\n\nÉtape 1 (MVP): bouton de déblocage.\nÉtape 2: achat Apple (StoreKit2) + restauration.")
                    .font(.body)
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
